...
function loadAbsoluteRPGAdventureSettings(settings, data) {
    ...
    if (settings.AbsoluteRPGAdventure != null) {
        ...
        console.info('Absolute RPG Adventure:', "ARA_local.loaded");
    } else {
        ARA_configReset();
    }
}
...
async function ARA_getResult(lastReply, chat_id, generate_data_prev, signal = null) {
    ...
    try {
        const res = await fetchWithTimeout(absoluteRPGAdventureUrl + '/getResult', ARA_local.config.request_result_timeout_ms, post);
